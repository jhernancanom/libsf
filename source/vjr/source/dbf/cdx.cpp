...
//////////////
	...
	u32 cdx_find_key(u32 tnDbfHandle, s32 tnTagIndex, u32 tnKeyLength)
	{
		...
		//////
			iiCdx_generateKey_byOps(wa, lsKeyOp, lnKeyOpCount, keyBuffer, true);
...
//		SCdxHeader*			head;
		...
		//////
			...
			for (lnTagNum = lnStartTagNum, llProcessed = false; lnTagNum <= lnEndTagNum && iCdx_getCompoundTagRoot(wa, wa->cdx_root, NULL, lnTagNum, &tagRoot); lnTagNum++)
			{
...
//////
	s32 iCdx_validateCdxKeys(SWorkArea* wa, u32 tnDbfHandle, s32 tnTagIndex, s8* tcMetaData, u32 tnMetaDataLength, s8* tcErrorsFound, u32 tnErrorsFoundLength)
	{
		...
		//////
			...
			lsKeyOp = iiCdx_generateKey_buildOps(wa, &tagHeader->keyExpression[0], &lnKeyOpCount);
			if (!lsKeyOp)
			{
		...
		//////
			for (lnRecno = 1, lnDbfKeyCount = 0; lnRecno <= wa->header.records; lnRecno++)
			{
				...
				// Load this record
				//////
					iDbf_gotoRecord(tnDbfHandle, lnRecno);
		...
		//////
			if (dbfKeys->populatedLength == 0)
			{
				...
				//////
					...
					if (iCdx_isDescending(tagHeader))		qsort(dbfKeys->buffer,	lnDbfKeyCount,	tagRoot.keyLength + 4,	iKeys_sortDescending);
					else									qsort(dbfKeys->buffer,	lnDbfKeyCount,	tagRoot.keyLength + 4,	iKeys_sortAscending);
			}
		...
		for (lnLkoNum = 0, lnKeyLength = 0, keyStart = keyExpression, lnFixup = _DBF_INDEX_FIXUP_NONE; *keyStart != 0; )
		{
			...
			//////
				...
				if (_memicmp(keyStart, "deleted()", 9) == 0)
				{
					...
					//////
						if (iDbf_getField_allowNulls(wa->thisWorkArea, lnFieldNumber, NULL, 0))
						{
							// Grab the offset and mask for rapid updating as we generate each field
							iDbf_getNull_offsetAndMask(wa->thisWorkArea, lnFieldNumber, &lko[lnLkoNum].null_offset, &lko[lnLkoNum].null_mask);
							++lnIndexLength;
						}
			...
			if (lko->indexFixup)
			{
				...
				switch (lko->indexFixup & _DBF_INDEX_FIXUP_MASK)
				{
					case _DBF_INDEX_FIXUP_SWAP_ENDIAN:
						...
						iiCdx_generateKey_byOps_fixup_swapEndian(lko, keyPart);
						break;
					...
					case _DBF_INDEX_FIXUP_DATE:
						...
						iiCdx_generateKey_byOps_fixup_date(lko, keyPart);
						// All done
						break;
					...
					case _DBF_INDEX_FIXUP_NUMERIC:
						...
						iiCdx_generateKey_byOps_fixup_numeric(lko, keyPart, wa);
						break;
				}
						...
						for (lnI = 0; lnI < lsFor->subitemCount; lnI++)
						{
							...
							switch (lfsi->type)
							{
								...
								case _FOR_CLAUSE_SUBITEM_TYPE_ALPHANUMERIC:
									...
									//////
										iDbf_getField_type(wa->thisWorkArea, lnFieldnum, fieldName, sizeof(fieldName));
									...
									//////
										...
										lfco->offsetL		= iDbf_getField_dataOffset(wa->thisWorkArea, lnFieldnum);
...
// END
//////////
	bool iCdx_getCompactIdxKey_byNumber(SCdxHeader* head, u32 keyLength, u8 tcFillChar, u32 tnKeyNumberTarget, u32 tnKeyNumberThis, SCdxNode* node, SCdxKey* key, STagRoot* tagRoot, bool tlFixupEndian, bool tlFixupSignBit)
	{
